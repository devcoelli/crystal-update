...
let HOWMUCH            = 0;
let TRANSIENT          = true;
let UPDATE_CMD         = "kgx -e '/bin/sh -c \"ame upg ; echo Done - Press enter to exit; read _\" ' ";
			...
			if (enabled && this._updateList.length > 0) {
				this._updateList.forEach( item => {
					if(DISABLE_PARSING) {
						...
						if (matches == null) {
							// Not an update
							this.menuExpander.menu.box.add_child( new St.Label({ text: item, style_class: 'crystal-updates-update-title' }) );
						} else {
							...
							if (!STRIP_VERSIONS) {
								hBox.add_child( new St.Label({
									...
									y_expand: true,
									y_align: Clutter.ActorAlign.CENTER,
									style_class: 'crystal-updates-update-version-from' }) );
								hBox.add_child( new St.Label({
									text: matches[3],
									style_class: 'crystal-updates-update-version-to' }) );
							}
	...
	_showNotification(title, message) {
		...
		if (this._notifSource == null) {
			...
			this._notifSource = new MessageTray.Source({
				...
				icon: this._getCustIcon("crystal-lit-symbolic"),
			});
		...
		this._notification.gicon = this._getCustIcon("crystal-updates-symbolic");
